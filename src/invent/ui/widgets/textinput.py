...
class TextInput(Widget):
    ...
    def __init__(self, **kwargs):
        self._number_of_lines = kwargs.pop("number_of_lines", 1)
        if self._number_of_lines < 1:
            raise ValueError(_("number_of_lines must be at least 1"))
    ...
    def on_value_changed(self):
        if self._number_of_lines == 1:
            ...
            # It's a textarea, so we set the value via innerText to preserve
            # newlines.
    ...
    def render(self):
        if self._number_of_lines == 1:
            ...
            # For multiple lines, we use a textarea element instead of input.
            # The input_type attribute is not applicable to textarea, so we
            # ignore it.
            element = textarea(id=self.id, rows=str(self._number_of_lines))
