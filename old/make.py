...
def _process_code(executable, use_python, *args):
    """Perform some action (check, test etc.) across the .py files
    ...
    for filepath in _walk("pypercard", INCLUDE_PATTERNS, EXCLUDE_PATTERNS):
        p = subprocess.run(execution + [filepath] + list(args))
        returncodes.add(p.returncode)
...
def pyflakes(*pyflakes_args):
    ...
    Call pyflakes on all .py files outside the docs directories
    """
    print("\npyflakes")
...
@export
def tidy():
    """Tidy the code with the 'black' formatter
    """
    print("\nTidy")
    return _process_code(BLACK, False, ("-l", "79", "pypercard"))
    return _process_code(BLACK, False, ("-l", "79", "tests"))
