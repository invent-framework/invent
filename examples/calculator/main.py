...
def when_any_button_is_clicked(message):
    ...
    if button.label in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]:
        data["numbers"] = data["value"] = data["value"].lstrip("0") + button.label

    elif button.label == "+/-":
        data["numbers"] = data["value"] = str(float(data["value"] or "0") * -1)

    elif button.label == "%":
        data["numbers"] = data["value"] = str(float(data["value"] or "0") / float(100))
        ...
        data["right"] = float(data["value"] or "0") # Decimal(data["value"] or "0")
        calculate(data)
...
invent.subscribe(when_any_button_is_clicked, to_channel="calculator", when_subject=["press"])
