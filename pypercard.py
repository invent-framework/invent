...
class Card:
    ...
    def show(self):
        ...
        self.content.innerHTML = self.template.format(**self.app.datastore)
        ...
        for transition in self._transitions:
            ...
            for element in target_elements:
                def handler(evt):
                    self.app.machine.next({"event": transition["event_name"], "dom_event": evt})
        ...
        if self.on_render:
            self.on_render(self, self.app.datastore)
    ...
    def _create_content(self):
        """Create the card's content (i.e. non-template element)."""

        element = document.createElement("div")
        element.setAttribute("pypercard", True)

        # Hide the card until we are ready :)
        element.style.display = "none"

        # TODO: Extract this so that the user can control card styling...
        element.style.margin = "20vh 20vw"
        element.style.min_width = "60vw"
        element.style.min_height = "60vh"
        element.style.textAlign = "center"

        # # Format the card template using values from the datastore.
        element.innerHTML = self.template.format(**self.app.datastore)
...
class App:
    ...
    def get_next_card(self, card):
        """Get the next card to (auto) advance to.
        ...
        if index == len(self.cards)-1:
            raise ValueError("Can't auto advance from the last card!")

        next_card = self.cards[index+1]
    ...
    def _create_auto_advance_transition(self, card):
        ...
        def timeout_acceptor(machine, input_):
        ...
        # next_card = self.get_next_card(card)
        # if not next_card:
        #     raise ValueError(f"Missing next card to auto-advance from: {card.name}")

        return Transition(
            source=card.name,
            target=target,
            acceptor=timeout_acceptor,
        )
...
class Machine:
    ...
    def __init__(
        ...
        self.transitions = [
            transition if isinstance(transition, Transition) else Transition(
                *transition)
