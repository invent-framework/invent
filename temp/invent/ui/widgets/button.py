...
class Button(Widget):
	text=TextProperty(_('The text on the button.'),default_value='Click Me');size=ChoiceProperty(_('The size of the button.'),default_value=_A,choices=['LARGE',_A,'SMALL'],group='style');purpose=ChoiceProperty(_("The button's purpose."),default_value='DEFAULT',choices=PURPOSES,group='style');press=Event(_('Sent when the button is pressed.'),button=_('The button that was clicked.'))
	@classmethod
	...
	def click(A,event):A.publish('press',button=A)
