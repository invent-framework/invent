...
class Meter(Widget):
	...
	def on_value_changed(A):A.element.value=A.value;A.publish(A.changed,value=A.value)
	def on_min_value_changed(A):A.element.min=A.min_value
	def on_max_value_changed(A):A.element.max=A.max_value
