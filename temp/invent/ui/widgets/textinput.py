...
class TextInput(Widget):
	...
	def on_keypress(A,event):A.publish(A.keypressed,key=event.key)
	def render(A):
		if A._number_of_lines==1:B=input_(type=A.input_type,id=A.id)
		else:B=textarea(id=A.id,rows=str(A._number_of_lines))
		B.addEventListener('input',create_proxy(A.on_input));B.addEventListener('keypress',create_proxy(A.on_keypress));return B