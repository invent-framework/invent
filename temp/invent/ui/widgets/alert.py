...
class Alert(Widget):
	title=TextProperty(_('The title of the alert.'));text=TextProperty(_('The text to display in the alert.'));purpose=ChoiceProperty(_('The purpose of the alert.'),default_value=_A,choices=PURPOSES,group='style');dismissable=BooleanProperty(_('Whether the alert can be dismissed by the user.'),default_value=False);dismissed=Event(_('An event that is fired when the alert is dismissed.'))
	@classmethod
	...
	def on_dismissed(A):A.element.remove();A.publish(A.dismissed)
