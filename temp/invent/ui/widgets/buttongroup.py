...
class ButtonGroup(Widget):
	choices=ListProperty(_('The options from which to select.'));value=TextProperty(_('The currently selected option.'),default_value=None);group=TextProperty(_('The group to which the button-group belongs.'),default_value='');size=ChoiceProperty(_('The size of the buttons.'),default_value=_A,choices=['LARGE',_A,'SMALL'],group='style');purpose=ChoiceProperty(_("The buttons' purpose."),default_value='DEFAULT',choices=PURPOSES,group='style');change=Event(_('Sent when the selected option changes.'),value=_('The new value of the button group.'));_items=OrderedDict()
	def _build_pair(A,index,choice):
		...
		C.addEventListener(_B,create_proxy(A._on_radio_change));E=label(B,for_=D,classes=['invent-btn']);return C,E
	...
	def _on_radio_change(A,event):B=event.target.value;A.value=B;A.publish(_B,value=B)
