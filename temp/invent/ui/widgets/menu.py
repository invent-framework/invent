...
class Menu(Widget):
	choices=ListProperty(_('The options from which to select.'));hover=ChoiceProperty(_('Defines where the menu appears in relation to the button.'),default_value='BELOW',choices=['ABOVE','BELOW','BEFORE','AFTER'],group=_B);text=TextProperty(_('The text on the button.'),default_value='≡');size=ChoiceProperty(_('The size of the button.'),default_value=_C,choices=['LARGE',_C,'SMALL'],group=_B);purpose=ChoiceProperty(_("The button's purpose."),default_value='DEFAULT',choices=PURPOSES,group=_B);opened=Event(_('Sent when the button is pressed.'),menu=_('The menu that was opened.'),button=_('The button that was clicked.'));selected=Event(_('The menu option has been selected.'),menu=_('The menu in which the selection was made.'),selected=_('The new selected option.'))
	@classmethod
	...
	def open_menu(A,event):
		...
		A.publish('opened',menu=A,button=A.trigger_button.element)
		def E(option):
			def B(e):e.stopPropagation();A._close_menu();A.publish('selected',menu=A,selected=option)
