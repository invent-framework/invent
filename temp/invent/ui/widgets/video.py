...
class Video(Widget):
	source=TextProperty(_('The video source file to play.'));playing=Event(_('Sent when the video starts to play.'),source=_('The video source playing.'));paused=Event(_('Sent when the video is paused.'),source=_('The video source paused.'),position=_('The pause position in seconds.'));position_changed=Event(_('Sent when the position in the video is changed.'),source=_('The video source that has been affected.'),position=_('The new position in seconds.'))
	@classmethod
	...
	def set_position(A,position):B=position;C=A.element.querySelector(_A);C.currentTime=B;A.publish('position_changed',source=A.source,position=B)
	def on_play(A,event):A.publish('playing',source=A.source)
	def on_pause(A,event):A.publish('paused',source=A.source,position=event.target.currentTime)
	def _build_native(B):
