...
class CodeEditor(Widget):
	...
	async def _emit_changed(A):await asyncio.sleep(_DEBOUNCE_DELAY);A._setting_code=True;A.code=A._view.state.doc.toString();A._setting_code=_B;A.publish('changed',code=A.code)
