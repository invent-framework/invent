...
class Modal(Widget):
	text=TextProperty(_('The text on the button.'),default_value='Click Me');size=ChoiceProperty(_('The size of the button.'),default_value=_B,choices=['LARGE',_B,'SMALL'],group='style');purpose=ChoiceProperty(_("The button's purpose."),default_value='DEFAULT',choices=PURPOSES,group='style');opened=Event(_('Sent when the button is pressed.'),button=_('The button that was clicked.'));closed=Event(_('Sent when the modal is dismissed.'))
