...
class Calendar(Widget):
	...
	def _day_cell(B,cell,today):
		A=cell['date'];C=cell[_B];D=['calendar-day']
		if C:D.append('has-events')
		if A==today:D.append('calendar-today')
		E=web.div(web.span(str(A.day),classes=['calendar-day-num']));E.addEventListener(_A,create_proxy(lambda e,date=A:B.publish('date_clicked',date=date)))
		if C:
			F=[]
			for(G,H)in C:I=web.li(web.time(B._format_time(G)),f" {H}");I.addEventListener(_A,create_proxy(lambda e,hm=G,description=H:B.publish('event_clicked',date=A,time=hm,content=description)));F.append(I)
