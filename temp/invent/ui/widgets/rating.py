...
class Rating(Widget):
	value=FloatProperty(_('The current rating value (multiples of 0.5).'),default_value=.0);maximum=ChoiceProperty(_('The number of stars to display.'),default_value='5',choices=['1','3','5','10'],group=_B);step=ChoiceProperty(_('The rating step size.'),default_value=_A,choices=[_A,'1'],group=_B);read_only=BooleanProperty(_('Prevent the user from changing the rating.'),default_value=False);show_label=BooleanProperty(_('Whether to show the numeric rating value next to the stars.'),default_value=True,group=_B);change=Event(_('Sent when the rating value is changed by the user.'),rating=_('The Rating widget whose value changed.'),value=_('The new rating value.'))
	@classmethod
	...
	def _click(A,value):
		...
		def C(event):
			...
			if not A.read_only:
				...
				A.publish('change',rating=A,value=A.value)
		return create_proxy(C)
	def _rebuild_stars(A):
		H='invent-rating-half';F='click';A._stars_element._dom_element.replaceChildren();I=int(A.maximum)
		...
		if A.show_label:J=f"{A.value}".replace('.0','');A._value_element.textContent=f"{J}/{A.maximum}"
		else:A._value_element.textContent=''
	def on_value_changed(A):
		if hasattr(A,_C):A._rebuild_stars()
	def on_maximum_changed(A):
		if A.value>int(A.maximum):A.value=float(int(A.maximum))
		if hasattr(A,_C):A._rebuild_stars()
	def on_read_only_changed(A):
		if hasattr(A,_C):
			...
			A._rebuild_stars()
	def render(A):
		A._stars_element=span();A._stars_element.classes.add('invent-rating-stars');A._value_element=span(f"{A.value}/{A.maximum}");A._value_element.classes.add('invent-rating-value');A._message_element=span('');A._message_element.classes.add('invent-rating-message');B=div(A._stars_element,A._value_element,A._message_element,id=A.id);B.classes.add('invent-rating')
